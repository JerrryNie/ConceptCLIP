...
def build_zero_shot_concept_classifier(
    ...
    def _process_batch(batch_classnames):
        ...
        for name in batch_classnames:
            ...
            for template in templates: # 对于每一个template，首先找出name中的词在template中(tokenized之后)的位置，然后取出对应位置的token的embedding，取平均
...
def evaluate_with_local(model, dataloader, global_classifier, local_classifier, topk, alpha,
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            local_core = torch.topk(concept_logit_scale * image_token_features @ local_classifier, k=topk, dim=1)[0].mean(dim=1) # 对于每一个concept(class label)，取topk个token的logit，然后取平均
...
class ConceptCLIPCLSEvalDataset(Dataset):
    def __init__(self,
                 ann_path,
                 data_path,
                 transform,
...
class ConceptCLIPMultiLabelCLSEvalDataset(Dataset):
    def __init__(self,
                 ann_path,
                 data_path,
                 transform,
