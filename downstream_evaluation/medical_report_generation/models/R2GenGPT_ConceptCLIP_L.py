...
from transformers import LlamaForCausalLM, LlamaTokenizer, AutoModel
...
class R2GenGPT(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.visual_encoder = AutoModel.from_pretrained("../../../pre_training/src/pretrained_checkpoint/ConceptCLIP", trust_remote_code=True).visual
        ...
        self.llama_proj = nn.Linear(1152, self.llama_model.config.hidden_size)
    ...
    def encode_img(self, images):
        ...
        for image in images:
            ...
            if self.hparams.global_only:
                image_embed = self.visual_encoder(image)[0].unsqueeze(1).to(device)
            else:
                image_embed = self.visual_encoder(image)[1].to(device)

            # image_embed = image_embed.view(image_embed.size(0), 27, 27, 1152)
            # image_embed = self.pooling(image_embed.permute(0, 3, 1, 2)).view(image_embed.size(0), 1152, -1)
