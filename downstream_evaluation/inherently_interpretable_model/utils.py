...
# NOTE: add ConceptCLIP
def prepare_img_feat_ConceptCLIP(img_names,
                     ckpt_path=None,
                     save_path=None,
                     clip_model_name='ConceptCLIP'): 
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    if clip_model_name == 'ConceptCLIP':
        model, preprocess, tokenizer = load_ConceptCLIP()
        latent_dim = 1152
    ...
    def process_img(img_names):
        ...
        with torch.no_grad():
            img_feat = model.encode_image(img_tensor) 
            local_img_feat = img_feat[1]  
            img_feat = img_feat[0]        
        return img_feat

    batchify_run(process_img, img_names, res, batch_size=64, use_tqdm=True) 
    if save_path:
        torch.save(res, save_path)
    return res  
...
# NOTE: add ConceptCLIP
def prepare_txt_feat_ConceptCLIP(prompts, ckpt_path=None, save_path=None, clip_model_name='ConceptCLIP'):
    ...
    def process_txt(prompts):
        ...
        with torch.no_grad():
            txt_feat = model.encode_text(token)
            local_txt_feat = txt_feat[1]
            txt_feat = txt_feat[0]

        return txt_feat

    batchify_run(process_txt, prompts, res, batch_size=128, use_tqdm=True)
