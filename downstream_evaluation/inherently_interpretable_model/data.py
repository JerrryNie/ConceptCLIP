...
class DotProductDataset(Dataset):
    ...
    def __init__(self, img_feat, txt_feat, label, on_gpu):
        self.dot_product = (img_feat @ txt_feat.t())  # (num_data, dim) @ (n_concept, dim).t() = (batch_size, n_concept)
        self.dot_product = self.dot_product.cuda(
        ) if on_gpu else self.dot_product
        self.labels = label.cuda() if on_gpu else label  # (num_data)
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.force_compute = force_compute # True
        self.use_txt_norm = use_txt_norm   # False
        self.use_img_norm = use_img_norm   # False
        self.use_cls_name_init = use_cls_name_init  # 'none'
        self.use_cls_sim_prior = use_cls_sim_prior  # 'none'
        self.remove_cls_name = remove_cls_name      # False
        ...
        # the dir to save all the image features of train/val/test set
        ...
        # the dir to save all the labels
        self.label_save_dir = {
            mode: self.img_split_path.joinpath(
        ...
        # the dir to save concept features
        ...
        if self.concept_select_fn is not None:  # select concept if not using pre-defined concepts
            ...
            print("Select all the pre-defined concepts.")
            self.select_idx = idx
    ...
    def prepare_txt_feat(self, concepts_raw, clip_model, clip_ckpt):
        ...
        if not self.concept_feat_save_dir.exists() or self.force_compute:
            if "ViT" in clip_model:
                print('prepare txt feat using CLIP')
                self.concept_feat = utils.prepare_txt_feat(concepts_raw,
                                                    clip_model_name=clip_model,
                                                    ckpt_path=None)
            # NOTE: ConceptCLIP
            elif "ConceptCLIP" in clip_model:  
                print('prepare txt feat using ConceptCLIP')
                self.concept_feat = utils.prepare_txt_feat_ConceptCLIP(concepts_raw,
                                                    clip_model_name=clip_model,
                                                    ckpt_path=None) 
    ...
    def compute_img_feat(self, cls2img, n_shots, clip_model, clip_ckpt):
        ...
        # NOTE: ConceptCLIP
            ...
            img_feat = utils.prepare_img_feat_ConceptCLIP(all_img_paths,
                                          clip_model_name=clip_model,
                                          ckpt_path=clip_ckpt)
