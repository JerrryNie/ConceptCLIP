...
#SBATCH --output=src/logs/srun_output/pretraining_first_stage_23M_multinodes_slurm.txt
#SBATCH --error=src/logs/srun_output/pretraining_first_stage_23M_multinodes_slurm.error.txt
...
srun torchrun --nnodes=6 --nproc_per_node 8 --master_addr=${MASTER_ADDR} --master_port=${MASTER_PORT} --rdzv_backend=c10d --rdzv_id=$RANDOM --rdzv_endpoint=${head_node_ip}:36899 -m open_clip_train.main \
    ...
    --seed 0 \
    --log-dir logs \
    --name 'pretraining_first_stage_23M' \
